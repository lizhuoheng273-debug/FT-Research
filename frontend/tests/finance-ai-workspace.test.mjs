...
function mountAiWorkspace({ query = "source=ai-news", from = "/ai/news", responses = {} } = {}) {
  const slots = [];
  const effects = [];
  const requests = [];
  const navigations = [];
...
test("AI-news story IDs are encoded in the detail request and missing stories stay unavailable", async () => {
  ...
  assert.equal(missing.sessionOptions.context, "来源：AI 热点资讯\n上下文暂不可用。");
