  ...
  return (
    <div className="flex min-h-screen items-center justify-center px-6">
      <div className="glass w-full max-w-md rounded-2xl p-7">
        ...
        <button onClick={() => void signInGuest().catch((reason) => setError(reason instanceof Error ? reason.message : "游客入口失败"))} className="mt-3 w-full rounded-lg border border-border px-3 py-2">游客体验</button>
        <p className="mt-4 text-xs text-muted-foreground">游客刷新或退出后无法找回本次临时记录。</p>
      </div>
    </div>
  );
}
