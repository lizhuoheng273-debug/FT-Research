  ...
  return <div>
    ...
    <article>
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_280px]">
        <GlassCard><h2 className="mb-2 text-base font-semibold">AI 导读</h2>{!digest && loading ? <div aria-label="AI 导读加载中" className="space-y-2 animate-pulse"><div className="h-4 w-full rounded bg-muted/60" /><div className="h-4 w-5/6 rounded bg-muted/60" /><div className="h-4 w-2/3 rounded bg-muted/60" /></div> : digest ? <div className="prose prose-sm max-w-none dark:prose-invert"><ReactMarkdown remarkPlugins={[remarkGfm]}>{digest}</ReactMarkdown></div> : null}{story?.latest && <div className="mt-4 rounded-lg border border-primary/20 bg-primary/5 p-3 text-sm"><b>最新进展：</b>{story.latest}</div>}<div className="mt-5 border-t border-border/50 pt-4"><AskAiButton context={context} workspaceSource="ai-news" workspaceEventId={storyId} workspaceSnapshot={{ title, summary: digest, latest: story?.latest, originalUrl: originalLink }} workspaceReturnState={location.state as Record<string, unknown> | null} label="AI 摘要与追问" suggestions={["这件事最值得关注的影响是什么？", "帮我梳理这件事的时间线"]} /></div></GlassCard>
