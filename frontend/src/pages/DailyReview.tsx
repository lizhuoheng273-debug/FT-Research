...
function EmotionDetails({ emotion }: { emotion: ShortTermEmotion }) {
  ...
  return <div className="space-y-5">
    ...
    <div className="grid grid-cols-3 gap-2">{rates.map(([label, value]) => <div key={label} className="rounded-lg bg-muted/20 p-3 text-center"><p className="text-xs text-muted-foreground">{label}</p><p className="mt-1 font-mono font-bold">{rate(value)}</p></div>)}</div>
...
export function DailyReview() {
  ...
  return <div>
    ...
    <div className="mb-6 grid items-start gap-4 lg:grid-cols-[minmax(0,2fr)_minmax(0,3fr)]">
      <button type="button" disabled={!emotion?.date} onClick={() => emotion && setModal("emotion")} className="glass w-full p-4 text-left transition-colors hover:border-primary/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">
