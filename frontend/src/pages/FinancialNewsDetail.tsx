...
export function FinancialNewsDetail() {
  ...
  const context = useMemo(() => event ? JSON.stringify({ title: event.title, sourceSummary: event.summary, aiDigest: event.aiDigest, urgencyReasons: event.urgencyReasons || event.scoreReasons, hotReasons: event.hotReasons || [], sources: event.relatedSources, stocks: event.relatedStocks }) : "", [event]);
  ...
  return <div>
    ...
    {!event && loading ? <p className="flex items-center justify-center gap-2 py-16 text-sm text-muted-foreground"><Loader2 className="h-4 w-4 animate-spin" />正在加载事件详情…</p> : event && <article>
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_300px]">
        <div className="space-y-4"><GlassCard><h2 className="mb-2 font-semibold">来源摘要</h2><p className="text-sm leading-7 text-muted-foreground">{event.summary || "暂无来源摘要，可打开原始来源查看完整内容。"}</p>{event.aiDigest && <div className="mt-4 rounded-xl border border-primary/20 bg-primary/5 p-3"><h3 className="text-xs font-semibold text-primary">AI 导读（辅助信息，请核对原文）</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">{event.aiDigest}</p></div>}<div className="mt-5 border-t border-border/50 pt-4"><AskAiButton context={context} label="AI 摘要与追问" suggestions={["这件事为什么重要？", "按时间梳理相关报道", "涉及哪些行业和公司？"]} /></div></GlassCard><GlassCard><h2 className="mb-3 font-semibold">相关报道时间线</h2><div className="space-y-3">{(event.reports || []).map((report, index) => <div key={report.id || index} className="border-l border-primary/30 pl-4"><p className="font-medium">{report.title}</p><p className="mt-1 text-xs text-muted-foreground">{report.source} · {report.publishedAt}</p>{report.summary && <p className="mt-1 text-sm text-muted-foreground">{report.summary}</p>}{report.originalUrl && <a href={report.originalUrl} target="_blank" rel="noreferrer" className="mt-1 inline-flex items-center gap-1 text-xs text-primary">原文 <ExternalLink className="h-3 w-3" /></a>}</div>)}</div></GlassCard></div>
        <aside className="space-y-4"><GlassCard><h2 className="mb-2 text-sm font-semibold">紧要分依据</h2><ul className="space-y-2 text-sm text-muted-foreground">{(event.urgencyReasons || event.scoreReasons)?.map((reason) => <li key={`urgent-${reason}`}>· {reason}</li>)}</ul><h2 className="mb-2 mt-5 border-t border-border/50 pt-4 text-sm font-semibold">热度分依据</h2><ul className="space-y-2 text-sm text-muted-foreground">{(event.hotReasons || []).map((reason) => <li key={`hot-${reason}`}>· {reason}</li>)}</ul></GlassCard><GlassCard><h2 className="mb-2 text-sm font-semibold">影响范围</h2><div className="flex flex-wrap gap-2"><span className="rounded-full bg-muted/60 px-2.5 py-1 text-xs">{event.category}</span>{event.relatedStocks?.map((code) => <span key={code} className="rounded-full bg-primary/10 px-2.5 py-1 text-xs text-primary">{code}</span>)}</div>{event.impactTags?.length ? <div className="mt-4 border-t border-border/50 pt-3"><p className="mb-2 text-[11px] text-muted-foreground">AI 影响标签（辅助信息）</p><div className="flex flex-wrap gap-2">{event.impactTags.map((tag) => <span key={tag} className="rounded-full bg-primary/5 px-2.5 py-1 text-xs text-muted-foreground">{tag}</span>)}</div></div> : null}</GlassCard><GlassCard><h2 className="mb-2 text-sm font-semibold">原始来源</h2><div className="space-y-2">{event.relatedSources?.map((source) => <p key={source} className="text-sm text-muted-foreground">{source}</p>)}</div></GlassCard></aside>
      </div>
    </article>}
  </div>;
}
